  ...
  // type (init_type)
   ...
   switch (type) {
     case PLAIN_INODE:
       ...
       wordvec_itor begin = make_shared<plain_file>(data)->begin();
       wordvec_itor end =  make_shared<plain_file>(data)->end();
       while ( begin != end ) size += begin++->size();
       size += make_shared<plain_file>(data)->size();
       break;
     case DIR_INODE:
       size = make_shared<directory>(dirents)->size();
       break;
   }
   DEBUGF ('i', "size = " << size);
   return size;
